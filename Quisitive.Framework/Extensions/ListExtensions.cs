...
namespace Quisitive.Framework.Extensions
{
   public static class ListExtensions
   {
      ...
      /// <returns></returns>
      public static IEnumerable<T> Sort<T>(Func<IEnumerable<T>> action, IComparer<T> comparer)
      {
         return action().Sort(comparer.Compare);
      }
      ...
      /// <param name="comparer">The comparer.</param>
      /// <returns></returns>
      public static IEnumerable<T> Sort<T>(Func<int, IEnumerable<T>> action, int id, IComparer<T> comparer)
      {
         return action(id).Sort(comparer.Compare);
      }
      ...
      public static void ForEach<TA, TB>(IEnumerable<TA> als, IEnumerable<TB> bls, Action<TA, TB> p)
      {
        ...
        public static List<List<T>> Split<T>(this List<T> items, int groupSize)
        {
            ...
            for (int i = 0; i < items.Count; i += groupSize)
